...
const { loaders: { inlineMarkdownImageFileLoader }, } = utils_2.getFileLoaderUtils();
...
async function readMetadataPath(metadataPath) {
    try {
        return await fs_extra_1.readFile(metadataPath, 'utf8');
    }
    catch (e) {
...
function createAssetsExportCode(assets) {
    ...
    function createAssetValueCode(assetValue) {
        ...
        if (typeof assetValue === 'string' && assetValue.startsWith('./')) {
            ...
            return `require("${inlineLoader}${utils_1.escapePath(assetValue)}").default`;
        }
        return undefined;
    }
...
const docusaurusMdxLoader = async function (fileString) {
    const callback = this.async();
    const filePath = this.resourcePath;
    const reqOptions = this.getOptions() || {};
    const { frontMatter, content: contentWithTitle } = utils_1.parseFrontMatter(fileString);
    const { content, contentTitle } = utils_1.parseMarkdownContentTitle(contentWithTitle, {
    ...
    const options = {
        ...
        remarkPlugins: [
            ...
            [transformImage_1.default, { staticDir: reqOptions.staticDir, filePath }],
            [transformLinks_1.default, { staticDir: reqOptions.staticDir, filePath }],
...
${JSON.stringify(frontMatter, null, 2)}`;
        if (!options.isMDXPartialFrontMatterWarningDisabled) {
            ...
            if (shouldError) {
                ...
                console.warn(chalk_1.default.yellow(errorMessage));
            }
        }
    }
    ...
    const assets = reqOptions.createAssets && metadata
        ...
        : undefined;
    const exportsCode = `
export const frontMatter = ${stringify_object_1.default(frontMatter)};
export const contentTitle = ${stringify_object_1.default(contentTitle)};
...
exports.default = docusaurusMdxLoader;
