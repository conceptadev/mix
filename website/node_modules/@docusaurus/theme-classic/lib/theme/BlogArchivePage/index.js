...
function Year({
  ...
  return <>
      <h3>{year}</h3>
      <ul>
        {posts.map(post => <li key={post.metadata.date}>
            <_Link.default to={post.metadata.permalink}>
...
function YearsSection({
  ...
  return <section className="margin-vert--lg">
      <div className="container">
        <div className="row">
          {years.map((_props, idx) => <div key={idx} className="col col--4 margin-vert--lg">
              <Year {..._props} />
            </div>)}
        </div>
      </div>
...
function BlogArchive({
  ...
  const description = (0, _Translate.translate)({
    id: 'theme.blog.archive.description',
    message: 'Archive',
    description: 'The page & hero description of the blog archive page'
  });
  const years = listPostsByYears(archive.blogPosts);
  return <_Layout.default title={title} description={description}>
