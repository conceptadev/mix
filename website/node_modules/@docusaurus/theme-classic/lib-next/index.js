...
const AnnouncementBarDismissDataAttribute =
  'data-announcement-bar-initially-dismissed'; // We always render the announcement bar html on the server, to prevent layout shifts on React hydration
...
export default function docusaurusThemeClassic(
  context, // TODO: LoadContext is missing some of properties
  options,
) {
  const {
    ...
    configurePostCss(postCssOptions) {
      if (direction === 'rtl') {
        ...
        const plugin = {
          ...
          prepare: (result) => {
            const file = result.root?.source?.input?.file; // Skip Infima as we are using the its RTL version.
