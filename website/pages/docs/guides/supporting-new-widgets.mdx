...
Note that the `shouldApplyModifiers` method was used in the `build` method before the `TextField`, it's a method that make available the `modifiers attributes` be applied to the widget.
