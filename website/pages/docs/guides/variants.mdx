...
When the `onDark` variant is applied, it will override the `box.color` and `text.style.color` attributes. When this happens the style will be equivalent to the following:
...
```dart
final contextVariant = ContextVariant(
      (context) => // Your condition here,
      priority: VariantPriority.normal // set the priority here,
    );
```
