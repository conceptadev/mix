...
After that, you simply need to use the variable `$token.{tokenCategory}` to access the design tokens. This is possible because the `TokenUtil` classes have already been instantiated within the Mix framework.
...
The code above illustrates a simple button created with Mix primitive widgets and previously defined tokens for styling. If you are already familiar with building components using Mix, there is only one thing you need to pay attention to: the attributes ending with the `of` method, which are used to access the token values.
