...
void main() {
  testWidgets('StyledContainer', (WidgetTester tester) async {
    ...
    await tester.pumpStyledWidget(
      StyledContainer(
        style: StyleMix(paddingAttr, marginAttr, backgroundColorAttr,
            alignmentAttr, clipAttr, boxDecorationAttr),
      ),
    );
    ...
    expect(containerWidget.clipBehavior, clipAttr.resolve(EmptyMixData));
    expect(containerDecoration.border,
        boxDecorationAttr.resolve(EmptyMixData).border);
    expect(containerDecoration.color,
        boxDecorationAttr.resolve(EmptyMixData).color);
    expect(containerDecoration.borderRadius,
        boxDecorationAttr.resolve(EmptyMixData).borderRadius);
  });
}
