...
try {
  ...
  compiler.addEventListener('load', () => {
    ...
    window.parent.postMessage(
      { type: 'mix-tailwinds:reference-ready', exampleId: example.id },
      '*',
    );
  });
  compiler.addEventListener('error', () => {
    ...
    window.parent.postMessage(
      { type: 'mix-tailwinds:reference-error', message },
      '*',
    );
  });
  ...
  window.parent.postMessage(
    { type: 'mix-tailwinds:reference-error', message: String(error) },
    '*',
  );
}
