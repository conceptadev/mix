...
const exampleWebRoot = path.join(mixTailwindsRoot, 'example', 'web');
...
test('all manifest entries map to the real HTML and Dart source regions', () => {
  ...
  const html = fs.readFileSync(
    path.join(mixTailwindsRoot, 'example', 'real_tailwind', 'advanced-examples.html'),
    'utf8',
  );
  const dart = fs.readFileSync(
    path.join(mixTailwindsRoot, 'example', 'lib', 'advanced_parity_preview.dart'),
    'utf8',
  );
